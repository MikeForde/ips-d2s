...
function resolveGraphQLId(data, fallbackId = null) {
...
function toApiResult(result, fallbackId = null) {
  return {
    id: resolveGraphQLId(result.body, result.id || fallbackId),
...
function mapArrayToApiResults(arr) {
  return (arr || []).map((item, index) => ({
    id: resolveGraphQLId(item, String(index)),
    status: 200,
