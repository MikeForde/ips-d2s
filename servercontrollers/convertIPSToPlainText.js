...
function convertIPSToPlainText(req, res) {
  try {
    let ipsBundle;
    ...
    // Now convert the JSON "ipsBundle" to your internal schema, then to Human Readable Plain Text
    const ipsRecord = convertIPSBundleToSchema(ipsBundle);
    const plainData = generateXMPPPlainText(ipsRecord);
    res.json(plainData);

  } catch (error) {
    console.error('Error converting IPS to Plain Text:', error);
    res.status(500).send('Error converting IPS to Plain Text');
  }
}

module.exports = { convertIPSToPlainText };